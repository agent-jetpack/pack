...
def get_default_model() -> BaseChatModel:
    ...
    Requires `ANTHROPIC_API_KEY` (or `OPENROUTER_API_KEY`) to be set in the
    environment.

    Pack enhancement: tries OpenRouter first (if `OPENROUTER_API_KEY` is set),
    then falls back to Anthropic `claude-sonnet-4-6`.

    Returns:
        Chat model instance — OpenRouter-backed or `ChatAnthropic`.
    """
...
def create_deep_agent(  # noqa: C901, PLR0912, PLR0915  # Complex graph assembly logic with many conditional branches
    ...
    *,
    ...
    # Pack harness middleware: hooks → cost → permissions → compaction → memory.
    # Wraps user middleware so Pack-specific concerns apply around user code.
    ...
    # Provider-specific middleware goes between user/Pack middleware and memory
