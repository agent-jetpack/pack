...
def resolve_model(model: str | BaseChatModel) -> BaseChatModel:
    ...
    # Pack enhancement: intercept provider/model slash format (e.g.
    # "deepseek/deepseek-chat") and route through Pack's custom OpenRouter
    # provider when an OPENROUTER_API_KEY is present. Pack's provider uses
    # langchain-openai against OpenRouter's OpenAI-compatible base URL, which
    # is a distinct code path from upstream's langchain-openrouter profile.
    if "/" in model and not model.startswith(("http://", "https://")) and ":" not in model:
        if os.environ.get("OPENROUTER_API_KEY"):
            ...
            return OpenRouterProvider(api_key=os.environ["OPENROUTER_API_KEY"]).create_model(model)
